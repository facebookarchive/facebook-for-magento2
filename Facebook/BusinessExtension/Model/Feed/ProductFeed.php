...
class ProductFeed
{
    ...
    private function getProductImages(Product $product)
    {
        ...
        foreach ($product->getMediaGalleryImages() as $img) {
            ...
            // @todo remove replace
            $additionalImages[] = str_replace('localhost', 'magento.com', $img['url']);
        }
        return [
    ...
    public function buildProductRequest(Product $product, $method = self::ATTR_UPDATE)
    {
        ...
        $url = str_replace('localhost', 'magento.com', $url);

        $images = $this->getProductImages($product);
        $imageUrl = $this->buildProductAttr(self::ATTR_IMAGE_URL, $images['main_image']);
        $imageUrl = str_replace('localhost', 'magento.com', $imageUrl);
        ...
        $request_data = [
            ...
            'commerce_tax_category' => $product->getFacebookTaxCategory() ?: 'FB_APRL',
        ...
        while ($count < $totalProductCount) {
            ...
            try {
                if (!empty($requests)) {
                    ...
                    //$this->_fbeHelper->log("Requests " . print_r($requests, true));
                    $response = $this->_fbeHelper->makeHttpRequest($requests, $access_token);
                    //$this->_fbeHelper->log("Product push response " . print_r($response, true));
